...
export default function Question({ curQ, curPage, clickHandler }) {
  ...
  return (
    <>
      ...
      <Button position={firstBtn} text={current.a[0].text} value={current.a[0].value} clickHandler={clickHandler} curPage={curPage} length={length}></Button>
      <Button color='#01471d' position={secondBtn} text={current.a[1].text} value={current.a[1].value} clickHandler={clickHandler} curPage={curPage} length={length}></Button>
    </>
  )
}
